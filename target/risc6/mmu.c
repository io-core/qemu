...
{
/*
    RISC6CPU *cpu = risc6_env_get_cpu(env);
...
/*
static void mmu_flush_pid(CPURISC6State *env, uint32_t pid)
{

    CPUState *cs = env_cpu(env);
    RISC6CPU *cpu = env_archcpu(env);
    int idx;
    MMU_LOG(qemu_log("TLB Flush PID %d\n", pid));

    for (idx = 0; idx < cpu->tlb_num_entries; idx++) {
        RISC6TLBEntry *entry = &env->mmu.tlb[idx];
...
/*
    ...
    for (i = 0; i < cpu->tlb_num_entries; i++) {
        RISC6TLBEntry *entry = &env->mmu.tlb[i];
